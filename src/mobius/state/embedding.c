...
MobiusState* mobius_new_state(void) {
    ...
    state->context = create_execution_context(256);  // 256 initial stack capacity
    if (!state->context) {
    ...
    state->context->env = state->global_env;  // Set initial environment
...
void mobius_free_state(MobiusState* state) {
    ...
    if (state->registry) {
     ...
     // ⬅️ ADD THIS: Free execution context
    if (state->context) {
        free_execution_context(state->context);
    }
...
int mobius_register_function(MobiusState* state,
    ...
    // Store function info
    state->custom_functions[state->custom_function_count].name = mobius_strdup(name);
    ...
    state->custom_functions[state->custom_function_count].description = description ? mobius_strdup(description) : NULL;
